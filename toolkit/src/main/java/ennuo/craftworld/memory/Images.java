...
public class Images {
  ...
  private static byte[] toDDS(BufferedImage image) {
    ...
    try {
        ...
        file.delete();
    } catch (IOException ex) { System.err.println("Failed to convert BufferedImage to DDS"); }
    return data;
  }
  ...
  public static Resource toTEX(BufferedImage image) {
   ...
   public static BufferedImage fromDDS(byte[] DDS) {
    ...
    } catch (Exception ex) { System.err.println("There was an error when converting DDS to BufferedImage."); }
    return null; 
  }
    
  public static ImageIcon getImageIcon(BufferedImage image) { return getImageIcon(image, 320, 320); }
  ...
  public static ImageIcon getGroupIcon(BufferedImage master) {
    ...
    } catch (IOException ex) { Logger.getLogger(Images.class.getName()).log(Level.SEVERE, null, ex); }
