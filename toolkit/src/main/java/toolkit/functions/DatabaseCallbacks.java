...
public class DatabaseCallbacks
{
    ...
    public static void zero()
    {
        ...
        {
            int result = JOptionPane.showConfirmDialog(null, "Are you sure you want to zero" +
                                                             " " +
                                                             "this?", "Confirm erase",
    ...
    public static FileEntry newEntry(byte[] data)
    {
        ...
        EntryDialogue dialogue = new EntryDialogue(Toolkit.INSTANCE, (FileDB) database, path,
            null);
    ...
    public static void newFolder()
    {
        ...
        if (selected == null)
            ...
            path = new TreePath(database.addNode(selected.getFilePath() + selected.getName() +
                                                 "/" + folder).getPath());
    ...
    public static void changeHash()
    {
        ...
        String hash = JOptionPane.showInputDialog(Toolkit.INSTANCE, "File Hash",
            "h" + entry.getSHA1().toString().toLowerCase());
        if (hash == null) return;
    ...
    public static void changeGUID()
    {
        ...
        String input = JOptionPane.showInputDialog(Toolkit.INSTANCE, "File GUID",
    ...
    public static void renameItem()
    {
        ...
        String path = JOptionPane.showInputDialog(Toolkit.INSTANCE, "Rename",
            entry.getPath());
        if (path == null) return;
    ...
    public static void copyItems(FileDB destination)
    {
        ...
        String[] options = new String[] { "Overwrite", "Skip", "Duplicate", "Overwrite All",
            "Skip " +
            "All" };
                ...
                if (!forceOverwrite)
                {
                    ...
                    if (copy.getPath().equals(entry.getPath()))
                        message = String.format("Path: %s\nGUID: %s\nThis entry " +
                                                "already exists in" +
                                                " the database, what do you want to " +
                                                "do?", entry.getPath(),
                            entry.getGUID());
                    else
                        message = String.format("Source Path: %s\nDestination Path: " +
                                                "%s\nGUID: " +
                                                "%s\nThis entry already exists in the" +
                                                " database, what do you want " +
                                                "to do?", entry.getPath(),
                            copy.getPath(), entry.getGUID());
                    ...
                    if (response == -1 || response == OPTION_SKIP)
                        continue; // Treat closing dialog as skip
                    else if (response == OPTION_DUPLICATE)
                    {
                        EntryDialogue dialogue = new EntryDialogue(Toolkit.INSTANCE,
                            destination, entry.getPath(), null);
        ...
        if (Config.instance.addToArchiveOnCopy && ResourceSystem.getArchives().size() > 1)
        {
            ...
            {
                int result = JOptionPane.showConfirmDialog(null, "Do you also want to " +
                                                                 "copy file " +
                                                                 "data to another " +
                                                                 "archive?", "Copy",
                    JOptionPane.YES_NO_OPTION);
    ...
    public static void duplicateItem()
    {
        ...
        EntryDialogue dialogue = new EntryDialogue(Toolkit.INSTANCE, (FileDB) database,
            source.getPath(), null);
