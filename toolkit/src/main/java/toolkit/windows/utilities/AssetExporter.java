...
public class AssetExporter extends JDialog
{
    ...
    public AssetExporter(FileEntry entry)
    {
        ...
        if (rootData == null)
        {
            JOptionPane.showMessageDialog(this, "Unable to obtain resource data, do you " +
                                                "have the " +
                                                "archive mounted?", "An error occurred",
        ...
        try { resource = new SerializedResource(rootData); }
        catch (Exception ex)
        {
            JOptionPane.showMessageDialog(this, "Resource was unable to deserialize!", "An error " +
                                                                                       "occurred"
                , JOptionPane.ERROR_MESSAGE);
            this.dispose();
            this.root = null;
        ...
        if (entry.getSource().getType().hasGUIDs())
            this.root = new Asset(new ResourceDescriptor((GUID) entry.getKey(),
                resource.getResourceType()));
        else
            this.root = new Asset(new ResourceDescriptor(entry.getSHA1(),
                resource.getResourceType()));
        ...
        for (Asset asset : descriptors)
        {
            ...
            boolean isLocked = type.equals(ResourceType.MUSIC_SETTINGS) ||
                               ...
                               type.equals(ResourceType.SCRIPT); // These don't technically
            // *have* to be
            // GUID, but they're mostly useless otherwise.
            asset.locked = isLocked;
            boolean shouldUseGUID =
                isLocked || (asset.descriptor.isGUID() && defaults.contains(asset.descriptor.getGUID()));
                                       ...
                                       MaterialLibrary remap)
    {
        ...
        if (asset.recursed || type == ResourceType.INVALID)
        {
            asset.recursed = true;
            if (asset.hashinate)
                return new ResourceDescriptor(SHA1.fromBuffer(asset.data),
                    asset.descriptor.getType());
            return new ResourceDescriptor((GUID) asset.entry.getKey(),
                asset.descriptor.getType());
        }

        SerializedResource resource = new SerializedResource(asset.data);
        ...
        if (resource.getSerializationType() != SerializationType.BINARY)
        {
            ...
            if (remap != null && resource.getSerializationType() == SerializationType.GTF_SWIZZLED || resource.getSerializationType() == SerializationType.GXT_SWIZZLED)
            {
                ...
                if (useGTF)
                {
                    textureData = Arrays.copyOfRange(textureData, 0x80,
                        textureData.length);
                    asset.data =
                        SerializedResource.compress(new SerializationData(textureData,
            ...
            else if (remap == MaterialLibrary.LBP1 && resource.getResourceType().equals(ResourceType.GTF_TEXTURE))
            {
                ...
                asset.data = SerializedResource.compress(new SerializationData(data));
            }
        ...
        for (int i = 0; i < dependencies.length; ++i)
        {
            ...
            if (dependencyAsset != null)
                resource.replaceDependency(dependencyDescriptor,
                    this.recurse(dependencyAsset,
                        assets, remap));
        }

        // if (resource.getResourceType() == ResourceType.PLAN && asset.hashinate && asset.entry
        // .GUID != -1)
        ...
        if (remap != MaterialLibrary.NONE)
        {
            ...
            if (shouldConvert && resource.getResourceType().equals(ResourceType.PLAN))
            {
                ...
                try
                {
                    ...
                    data = SerializedResource.compress(plan.build());
                }
                catch (Exception ex)
                {
            ...
            else if (shouldConvert && resource.getResourceType().getCompressable() != null)
            {
                ...
                {
                    System.out.println("Failed to convert a " + resource.getResourceType() + " " +
                                       "resource!");
                    data = resource.compress();
                }
            }
    ...
    private void getDescriptors(byte[] resource, HashSet<Asset> descriptors)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            descriptor = new ResourceDescriptor(guid, sha1,
                ResourceType.fromType(data.i32()));
    ...
    private void updateButtonStates()
    {
        ...
        this.switchReferenceTypeButton.setText("Switch to " + ((asset.hashinate) ? "GUID" :
                                                                   "HASH"
        ));
                    ...
                    layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addComponent(exportButton,
                                ...
                                javax.swing.GroupLayout.DEFAULT_SIZE,
                                Short.MAX_VALUE)
                            .addComponent(materialLibraryCombo,
                                javax.swing.GroupLayout.Alignment.LEADING, 0,
                                javax.swing.GroupLayout.DEFAULT_SIZE,
                                Short.MAX_VALUE)
                            .addComponent(packagingTypeCombo,
                                javax.swing.GroupLayout.Alignment.LEADING, 0,
                                javax.swing.GroupLayout.DEFAULT_SIZE,
                                Short.MAX_VALUE)
                            ...
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(switchReferenceTypeButton,
                                    javax.swing.GroupLayout.DEFAULT_SIZE,
                                    javax.swing.GroupLayout.DEFAULT_SIZE,
                                    Short.MAX_VALUE)
                                .addGap(5, 5, 5)
                                .addComponent(markAllAsHashButton,
                                    javax.swing.GroupLayout.PREFERRED_SIZE,
                                    javax.swing.GroupLayout.DEFAULT_SIZE,
                                    javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(5, 5, 5)
                                .addComponent(markAllAsGUIDButton,
                                    javax.swing.GroupLayout.PREFERRED_SIZE,
                                    125,
                                    javax.swing.GroupLayout.PREFERRED_SIZE)))
                        .addContainerGap())
    ...
    private void markAllAsGUIDButtonActionPerformed(java.awt.event.ActionEvent evt)
    {//GEN-FIRST:event_markAllAsGUIDButtonActionPerformed
        for (int i = 0; i < this.assetModel.size(); ++i)
        {
            Asset asset = this.assetModel.getElementAt(i);
            if (!asset.locked)
    ...
    private void markAllAsHashButtonActionPerformed(java.awt.event.ActionEvent evt)
    {//GEN-FIRST:event_markAllAsHashButtonActionPerformed
        for (int i = 0; i < this.assetModel.size(); ++i)
        {
            Asset asset = this.assetModel.getElementAt(i);
            if (!asset.locked)
    ...
    private void switchReferenceTypeButtonActionPerformed(java.awt.event.ActionEvent evt)
    {//GEN-FIRST:event_switchReferenceTypeButtonActionPerformed
        int index = this.assetList.getSelectedIndex();
        if (index == -1) return;
        Asset asset = this.assetModel.getElementAt(index);
