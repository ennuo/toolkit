...
public class GfxGUI extends javax.swing.JFrame
{
    public static class TextureEntry
    {
        ...
        public String toString()
        {
            if (this.descriptor == null)
                return String.format("[%d] Empty texture slot", this.index);
    ...
    private void maxPriorityCheckboxActionPerformed(java.awt.event.ActionEvent evt)
    {//GEN-FIRST:event_maxPriorityCheckboxActionPerformed
    ...
    private String fixupEnvVar(String shader)
    {
        ...
        shader = shader.replace("ENV.REFRACTIVE_INDEX", String.format(Locale.ROOT, "%f",
            this.gmat.refractiveIndex));

        shader = shader.replace("ENV.FRESNEL_FALLOFF_POWER", String.format(Locale.ROOT, "%f",
            this.gmat.refractiveFresnelFalloffPower));
        shader = shader.replace("ENV.FRESNEL_MULTIPLIER", String.format(Locale.ROOT, "%f",
            this.gmat.refractiveFresnelMultiplier));
        shader = shader.replace("ENV.FRESNEL_OFFSET", String.format(Locale.ROOT, "%f",
            this.gmat.refractiveFresnelOffset));
        shader = shader.replace("ENV.FRESNEL_SHIFT", String.format(Locale.ROOT, "%f",
            this.gmat.refractiveFresnelShift));

        shader = shader.replace("ENV.FUZZ_LIGHTING_BIAS", String.format(Locale.ROOT, "%f",
            ((float) ((int) this.gmat.fuzzLightingBias & 0xff)) / 255.0f));
        shader = shader.replace("ENV.FUZZ_LIGHTING_SCALE", String.format(Locale.ROOT, "%f",
            ((float) ((int) this.gmat.fuzzLightingScale & 0xff)) / 255.0f));

        shader = shader.replace("ENV.IRIDESCENCE_ROUGHNESS", String.format(Locale.ROOT, "%f",
            ((float) ((int) this.gmat.iridesenceRoughness & 0xff)) / 255.0f));
    ...
    {//GEN-FIRST:event_compileButtonActionPerformed
        if (!System.getProperty("os.name").toLowerCase().contains("win"))
        {
            JOptionPane.showMessageDialog(this, "This program is only functional on Windows!",
                "Error", JOptionPane.WARNING_MESSAGE);
        ...
        if (this.receiveSpritelightCheckbox.isSelected())
            flags |= GfxMaterialFlags.RECEIVE_SPRITELIGHTS;
        ...
        if (this.noInstanceTextureCheckbox.isSelected())
            flags |= GfxMaterialFlags.NO_INSTANCE_TEXTURE;
        ...
        {
            new ErrorDialogue(this, true, "An error occurred while compiling BRDF shader.",
                ex.getMessage());
            return;
        }
        ...
        if (FileIO.write(resource, file.getAbsolutePath()))
            JOptionPane.showMessageDialog(this, "Succesfully compiled shader!", "Success!",
                JOptionPane.PLAIN_MESSAGE);
        else
            JOptionPane.showMessageDialog(this, "An error occurred writing file, is this " +
                                                "path " +
                                                "valid?", "Error",
                JOptionPane.WARNING_MESSAGE);
    }//GEN-LAST:event_compileButtonActionPerformed

    private void editTextureButtonActionPerformed(java.awt.event.ActionEvent evt)
    {//GEN-FIRST:event_editTextureButtonActionPerformed
    ...
    private void openBRDFButtonActionPerformed(java.awt.event.ActionEvent evt)
    {//GEN-FIRST:event_openBRDFButtonActionPerformed
