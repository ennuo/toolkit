...
public class ShapeAssembler
{
    ...
    public static void skeletate(Thing[] boneThings, Bone[] bones, Bone bone, Thing parentOrRoot)
    {
        ...
        if (bone.shapeVerts.length != 0)
        {
            ...
            shape.thickness =
                (float) (Math.round((Math.abs(bone.shapeMaxZ) + Math.abs(bone.shapeMinZ)) / 2.0));
    ...
    public static Thing[] computeBoneThings(Thing root, Matrix4f transform, Bone[] bones)
    {
        ...
        transform = transform.rotate((float) Math.toRadians(-90.0f), new Vector3f(1.0f, 0.0f,
            0.0f), new Matrix4f());
        ...
        for (int i = 0; i < bones.length; ++i)
        {
            ...
            if (i != 0)
            {
                ...
                // Matrix4f localPos = bone.skinPoseMatrix.mul(bones[0]
                // .invSkinPoseMatrix, new
                // Matrix4f());
                thing = new Thing(++UID);
    ...
    public static void main(String[] args)
    {
        ...
        plan.inventoryData.userCreatedDetails = new UserCreatedDetails("Generated Model",
            "Maybe " +
            "you should replace this descirption");

        plan.revision = new Revision(0x272, 0x4c44, 0x0008);
        plan.compressionFlags = 0x0;

        ArrayList<Thing> things = new ArrayList<>();
        Collections.addAll(things, boneThings);
        ...
        FileIO.write(SerializedResource.compress(plan.build()), args[2]);
    }
}
