...
public class Psyncer
{
      ...
      public static void main(String[] args)
      {
            ...
            try { local = archive.loadResource(archive.getKey().getRootHash(),
                    RLocalProfile.class); }
            catch (Exception ex)
            {
            ...
            try { synced = archive.loadResource(local.syncedProfile.getSHA1(),
                    RSyncedProfile.class); }
            catch (Exception ex)
            {
            ...
            try
            {
                  synced = GsonUtils.fromJSON(new String(jsonData, StandardCharsets.UTF_8),
                          RSyncedProfile.class);
            }
            catch (Exception ex)
            {
            ...
            local.syncedProfile = new ResourceDescriptor(
                    archive.add(SerializedResource.compress(synced, archive.getGameRevision(),
                            CompressionFlags.USE_NO_COMPRESSION)),
                    ResourceType.SYNCED_PROFILE
            );

            archive.getKey().setRootHash(archive.add(SerializedResource.compress(local,
                    archive.getGameRevision(), CompressionFlags.USE_NO_COMPRESSION)));
