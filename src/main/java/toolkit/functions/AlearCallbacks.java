...
public class AlearCallbacks {
    ...
    public static SHA1 sendResourceDataMessage(Socket client, byte[] data, String name) throws Exception
    {
        if (data == null || name == null || name.isEmpty()) return null;

        MemoryOutputStream packet = new MemoryOutputStream(32 + data.length + name.length() + 1);
    ...
    public static void sendAddPlanMessage(Socket client, GUID guid, SHA1 sha1) throws Exception
    {
