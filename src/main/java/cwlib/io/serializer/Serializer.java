...
public class Serializer {
    ...
    public MemoryInputStream input;
    public MemoryOutputStream output;
    ...
    public HashSet<ResourceReference> dependencies = new HashSet<>();
    ...
    public Serializer(MemoryInputStream data) {
        this.input = data;
    ...
    public Serializer(MemoryOutputStream data) {
        this.output = data;
        this.revision = data.revision;
        this.compressionFlags = data.compressionFlags;
    ...
    public Serializer(int size, Revision revision, byte compressionFlags) {
        this.isWriting = true;
        this.output = new MemoryOutputStream(size, revision);
    ...
    public Serializer(byte[] data, Revision revision, byte compressionFlags) {
        this.isWriting = false;
        this.input = new MemoryInputStream(data, revision);
    ...
    public ResourceReference resource(ResourceReference value, ResourceType type) {
        if (this.isWriting) {
            this.output.resource(value);
            return value;
        }
        ResourceReference descriptor = this.input.resource(type);
    ...
    public ResourceReference resource(ResourceReference value, ResourceType type, boolean useSingleByteFlag) {
        if (this.isWriting) {
            this.output.resource(value, useSingleByteFlag);
            return value;
        }
        ResourceReference descriptor = this.input.resource(type, useSingleByteFlag);
        if (descriptor != null)
            this.dependencies.add(descriptor);
        return descriptor;
    }
