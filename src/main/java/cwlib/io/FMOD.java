...
import cwlib.io.FMOD.FMODSampleBank.FMODSoundFormat;
...
public class FMOD {
    ...
    public static String[] CONSOLES = {"PC", "XBOX", "XBOX360", "GC", "PS2", "PSP", "PS3", "WII"};
    ...
    public static class FMODEventFile {
        ...
        public static enum EventMode { _2D, _3D }
        public static enum Rollof3DType { LOGARITHMIC, LINEAR, CUSTOM, UNSPECIFIED }
        public static enum Position3DType { WORLD_RELATIVE, HEAD_RELATIVE }
        public static enum StreamingType { 
            DECOMPRESS_INTO_MEMORY, 
            LOAD_INTO_MEMORY, 
            STREAM_FROM_DISK 
        }
        public static enum PlayMode { SEQUENTIAL, RANDOM, RANDOM_NO_REPEAT, SEQUENTIAL_NO_REPEAT, SHUFFLE, PROGRAMMER_SELECTED }
        public static enum PropertyType { INT, FLOAT, STRING }
        ...
        public static class Property {
            ...
            public String toXML() {
                ...
                if (this.type == PropertyType.INT)
                    xml += tag("data_int", (int) this.value);
                else if (this.type == PropertyType.FLOAT)
                    xml += tag("data_float", (float) this.value);
                else if (this.type == PropertyType.STRING)
                    xml += tag("data_string", (String) this.value);
        ...
        public static class SoundBank {
            ...
            public String getFooterXML(String format) {
                ...
                for (String console : CONSOLES) {
                    ...
                    xml += tag("_" + console + "_forcesoftware", (console.equals("PC") || console.equals("XBOX360") || console.equals("PS3")) ? "1" : "0");
        ...
        public static class EventCategory {
            ...
            public String toXML() {
                ...
                switch (this.maxPlaybacksBehavior) {
                    case 1: behavior = "Steal_newest"; break;
                    case 2: behavior = "Steal_quietest"; break;
                    case 3: behavior = "Just_fail"; break;
                    case 4: behavior = "Just_fail_if_quietest"; break;
                    default: behavior = "Steal_oldest"; break;
                }
        ...
        public static class EventEnvelopePoint {
            ...
            public String toXML(int index) {
                return tag("point", this.x + "," + this.y + "," + (index == 0 ? "1" : "0") + "," + this.curveShape);
            }
        }
        ...
        public static class EventEnvelope {
            public static enum EffectType {
                ...
                private EffectType(String name, int flags) {
                    this.flags = flags;
                    this.name = name;
                }
            ...
            public String toXML(String name, ArrayList<EventEnvelope> envelopes, ArrayList<EventParameter> parameters) {
                EventEnvelope parent = null;
                if (this.parentIndex != -1)
                    parent = envelopes.get(this.parentIndex);
                
                EventEnvelope envelope = parent != null ? parent : this;
                String dspName = this.effect != EffectType.DSP_EFFECT ? this.effect.getName() : envelope.name;
                ...
                xml += tag("parametername", parameters.get(this.controlParameterIndex).name);
                xml += tag("controlparameter", parameters.get(this.controlParameterIndex).name);
                for (String console : CONSOLES)
        ...
        public static class Layer {
            ...
            public String toXML(String layerName, ArrayList<EventParameter> parameters, ArrayList<SoundDef> soundDefs) {
                String xml = "<layer>\n";
                ...
                if (this.controlParameter != -1)
                    xml += tag("controlparameter", parameters.get(this.controlParameter).name);
                ...
                for (int i = 0; i < this.envelopes.size(); ++i)
                    xml += this.envelopes.get(i).toXML("env00" + i, this.envelopes, parameters);
                for (String console : CONSOLES)
                    xml += tag("_" + console + "_enable", 1);
        ...
        public static class Event {
            ...
            public String toXML(ArrayList<SoundDef> soundDefs) {
                ...
                for (int i = 0; i < this.layers.size(); ++i)
                    xml += this.layers.get(i).toXML("layer0" + i, this.parameters, soundDefs);
                ...
                xml += tag("volume_randomization", fieldRatioToDecibel(1.0f - this.volumeRandomization));
                xml += tag("priority", this.priority);
                xml += tag("maxplaybacks", this.maxPlaybacks);
                String behavior;
                switch (this.maxPlaybacksBehavior) {
                ...
                xml += tag("headrelative", position3D == 0x04 ? "Head_relative" : "World_relative");
                ...
                xml += tag("cone_outside_volumedb", fieldRatioToDecibel(this.coneOutsideVolume3D));
                ...
                xml += tag("spawn_intensity_randomization", this.spawnIntensityRandomization);
        ...
        public static class SoundDefWaveform {
            ...
            public String toXML() {
                ...
                if (this.isOscillator) {
                    ...
                    String type = (new String[] { "Sine", "Square", "Saw up", "Saw down", "Triangle", "Noise" })[this.waveType];
                    xml += tag("type", type);
        ...
        public static class SoundDefProperty {
            ...
            public String toXML() {
                String xml = tag("type", (new String[] { "sequentialnoeventrestart", "random", "randomnorepeat", "sequential", "shuffle", "programmerselected", "shuffleglobal"})[this.playMode]);
        ...
        private HashSet<Integer> usedSoundDefIndices = new HashSet<>();
        ...
        private EventSound readEventSound(MemoryInputStream stream) {
            ...
            usedSoundDefIndices.add((int) (instance.soundDefIndex & 0xffff) );
            ...
            if (isSimpleEvent) {
                ...
                for (int i = 0; i < numLayers; ++i) {
                    ...
                    for (int j = 0; j < envelopeCount; ++j) {
                        ...
                        envelope.effect = EffectType.fromFlags(combinedFlags & ~0x01 & 0xffff);
                    ...
                    for (int j = 0; j < paramCount; ++j) {
                        ...
                        int info = stream.i32(); // technically a difference when < 0x10, but dont need that much attention to detail
        ...
        public FMODEventFile(String file) {
            MemoryInputStream stream = new MemoryInputStream(file, CompressionFlags.USE_NO_COMPRESSION);
        ...
        public String toFDP(String bankDir, String outputDir) {
            ...
            for (String console : CONSOLES) {
                ...
                xml.append(tag("_" + console + "_forcesoftware", (console.equals("PC") || console.equals("XBOX360") || console.equals("PS3")) ? "1" : "0"));
            ...
            for (SoundBank bank : this.waveBanks) {
                ...
                if (file.exists()) {
                    ...
                    for (int i = 0; i < fsb.samples.size(); ++i) {
                        ...
                        SoundDefWaveform waveform = findWaveform(bank.name, i, sample.name);
    ...
    public static class FMODSampleBank {
        public static enum FMODSoundFormat {
            NONE,
            PCM8,
            PCM16,
            PCM24,
        ...
        public static final int FSOUND_LOOP_NORMAL = 0x00000002; /* For forward looping samples. */
        public static final int FSOUND_LOOP_BIDI = 0x00000004; /* For bidirectional looping samples. (no effect if in hardware). */
        ...
        public static final int FSOUND_UNSIGNED = 0x00000080; /* For user created source data containing unsigned samples. */
        public static final int FSOUND_SIGNED = 0x00000100; /* For user created source data containing signed data. */
        public static final int FSOUND_DELTA = 0x00000200; /* For user created source data stored as delta values. */
        public static final int FSOUND_IT214 = 0x00000400; /* For user created source data stored using IT214 compression. */
        public static final int FSOUND_IT215 = 0x00000800; /* For user created source data stored using IT215 compression. */
        public static final int FSOUND_HW3D = 0x00001000; /* Attempts to make samples use 3d hardware acceleration. (if the card supports it) */
        public static final int FSOUND_2D = 0x00002000; /* Tells software (not hardware) based sample not to be included in 3d processing. */
        public static final int FSOUND_STREAMABLE = 0x00004000; /* For a streamimg sound where you feed the data to it. */
        public static final int FSOUND_LOADMEMORY = 0x00008000; /* "name" will be interpreted as a pointer to data for streaming and samples. */
        public static final int FSOUND_LOADRAW = 0x00010000; /* Will ignore file format and treat as raw pcm. */
        public static final int FSOUND_MPEGACCURATE = 0x00020000; /* For FSOUND_Stream_Open - for accurate FSOUND_Stream_GetLengthMs/FSOUND_Stream_SetTime. WARNING, see FSOUND_Stream_Open for inital opening time performance issues. */
        public static final int FSOUND_FORCEMONO = 0x00040000; /* For forcing stereo streams and samples to be mono - needed if using FSOUND_HW3D and stereo data - incurs a small speed hit for streams */
        public static final int FSOUND_HW2D = 0x00080000; /* 2D hardware sounds. allows hardware specific effects */
        public static final int FSOUND_ENABLEFX = 0x00100000; /* Allows DX8 FX to be played back on a sound. Requires DirectX 8 - Note these sounds cannot be played more than once, be 8 bit, be less than a certain size, or have a changing frequency */
        public static final int FSOUND_MPEGHALFRATE = 0x00200000; /* For FMODCE only - decodes mpeg streams using a lower quality decode, but faster execution */
        public static final int FSOUND_IMAADPCM = 0x00400000; /* Contents are stored compressed as IMA ADPCM */
        public static final int FSOUND_VAG = 0x00800000; /* For PS2 only - Contents are compressed as Sony VAG format */
        //public static final int FSOUND_NONBLOCKING      0x01000000 /* For FSOUND_Stream_Open/FMUSIC_LoadSong - Causes stream or music to open in the background and not block the foreground app. See FSOUND_Stream_GetOpenState or FMUSIC_GetOpenState to determine when it IS ready. */
        public static final int FSOUND_XMA = 0x01000000;
        public static final int FSOUND_GCADPCM = 0x02000000; /* For Gamecube only - Contents are compressed as Gamecube DSP-ADPCM format */
        public static final int FSOUND_MULTICHANNEL = 0x04000000; /* For PS2 and Gamecube only - Contents are interleaved into a multi-channel (more than stereo) format */
        public static final int FSOUND_USECORE0 = 0x08000000; /* For PS2 only - Sample/Stream is forced to use hardware voices 00-23 */
        public static final int FSOUND_USECORE1 = 0x10000000; /* For PS2 only - Sample/Stream is forced to use hardware voices 24-47 */
        public static final int FSOUND_LOADMEMORYIOP = 0x20000000; /* For PS2 only - "name" will be interpreted as a pointer to data for streaming and samples. The address provided will be an IOP address */
        public static final int FSOUND_IGNORETAGS = 0x40000000; /* Skips id3v2 etc tag checks when opening a stream, to reduce seek/read overhead when opening files (helps with CD performance) */
        public static final int FSOUND_STREAM_NET = 0x80000000; /* Specifies an internet stream */
        ...
        public static byte[] fromAudioFile(String path) {
            ...
            ProcessBuilder builder = new ProcessBuilder("ffmpeg", "-y", "-i", path, destination.getAbsolutePath(), "-nostats", "-loglevel", "0");
        ...
        public FMODSampleBank(String path) {
            MemoryInputStream stream = new MemoryInputStream(path, CompressionFlags.USE_NO_COMPRESSION);
            stream.setLittleEndian(true);
        ...
        public byte[] extract(int index) {
            ...
            if (codec == FMODSoundFormat.IMAAPCM) {
                MemoryOutputStream stream = new MemoryOutputStream(sample.compressedDataLength + 0x1000);
                stream.setLittleEndian(true);
                ...
                stream.bytes(Arrays.copyOfRange(this.sampleData, offset, offset + sample.compressedDataLength));
            ...
            byte[] mp3 = Arrays.copyOfRange(this.sampleData, offset, offset + sample.compressedDataLength);
    ...
    public static void main(String[] args) throws Exception {
        ...
        // FMODEventFile file = new FMODEventFile("F:/ps3/game/NPUA80662/USRDIR/gamedata/audio/ps3_main.fev");
        // FileIO.write(file.toFDP("F:/ps3/game/NPUA80662/USRDIR/gamedata/audio", "E:/art/.lbp2/audio").getBytes(), "E:/art/.lbp2/audio/main.fdp");
        // file.extract("E:/scratch/fmod", "E:/art/audio");

        // FMODEventFile file = new FMODEventFile("C:/users/aidan/desktop/ps3_move.fev");
        // FileIO.write(file.toFDP("C:/users/aidan/desktop", "C:/users/aidan/desktop").getBytes(), "c:/users/aidan/desktop/move.fdp");
        ...
        //     while (file.exists()) {
        //         name = fsb.samples.get(i).name.split("\\.")[0] + String.format("_%02d", dupeIndex) + ".wav";
