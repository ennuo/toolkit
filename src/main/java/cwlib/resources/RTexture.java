...
public class RTexture {
    ...
    public static byte[] getDDSHeader(int format, int width, int height, int mips) {
        ...
        header.u32LE(RTexture.DDS_HEADER_FLAGS_TEXTURE | ((mips != 0) ? RTexture.DDS_HEADER_FLAGS_MIPMAP : 0));
        header.u32LE(height);
        header.u32LE(width);
        ...
        switch (format) {
            case 0x81: pixelFormat = RTexture.DDSPF_B8; break;
            case 0x82: pixelFormat = RTexture.DDSPF_A1R5G5B5; break;
            case 0x83: pixelFormat = RTexture.DDSPF_A4R4G4B4; break;
            case 0x84: pixelFormat = RTexture.DDSPF_R5G6B5; break;
            case 0x85: pixelFormat = RTexture.DDSPF_A8R8G8B8; break;
            case 0x86: pixelFormat = RTexture.DDSPF_DXT1; break;
            case 0x87: pixelFormat = RTexture.DDSPF_DXT3; break;
            case 0x88: pixelFormat = RTexture.DDSPF_DXT5; break;
            default: throw new Error("Unknown or unimplemented DDS Type!");
        }
        for (int value : pixelFormat)
            header.u32LE(value);
        
        int surfaceFlags = RTexture.DDS_SURFACE_FLAGS_TEXTURE;
        if (mips != 0) surfaceFlags |= RTexture.DDS_SURFACE_FLAGS_MIPMAP;
