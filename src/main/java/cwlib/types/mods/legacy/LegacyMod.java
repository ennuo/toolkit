package cwlib.types.mods.legacy;

import cwlib.util.Bytes;
import cwlib.io.streams.MemoryInputStream;
import cwlib.util.Images;
import cwlib.types.mods.legacy.patches.ModPatch;
import cwlib.enums.ModCompatibility;
...
public class LegacyMod {
    ...
    public ArrayList<InventoryItemDetails> items = new ArrayList<InventoryItemDetails>();
    ...
    public LegacyMod() {}
    public LegacyMod(File file, MemoryInputStream data, String key) { process(data, key); }
    
    private void process(MemoryInputStream data, String key) {
        ...
        if (itemCount != 0) {
            items = new ArrayList<InventoryItemDetails>(itemCount);
            for (int i = 0; i < itemCount; ++i) {
                InventoryItemDetails item = new Serializer(data).struct(null, InventoryItemDetails.class);
