...
public class SoftbodyCluster
{
    ...
    public float[] getRestQuadraticDyadicSum()
    {
    ...
    public void setName(String name)
    {
        if (name != null && name.length() > MAX_CLUSTER_NAME_LENGTH)
            throw new IllegalArgumentException("Cluster name cannot be longer than 32 " +
                                               "characters.");
        this.name = name;
    }

    public void setRestCenterOfMass(Vector4f COM)
    {
        this.restCenterOfMass = COM;
    }

    public void setRestDyadicSum(Matrix4f sum)
    {
    ...
    public void setRestQuadraticDyadicSum(float[] sum)
    {
        if (sum == null || sum.length != QUAD_DYADIC_SUM_LENGTH)
            throw new IllegalArgumentException("Rest quadratic dyadic sum array length must" +
                                               " be " +
                                               "81!");
