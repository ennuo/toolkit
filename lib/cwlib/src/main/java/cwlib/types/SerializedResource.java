...
public class SerializedResource
{
            ...
            case ENCRYPTED_BINARY:
                ...
                if (head >= 0x109)
                {
                    ...
                    if (head >= 0x189)
                    {
                        if (this.type != ResourceType.STATIC_MESH)
                        {
                            ...
                            this.meshInfo =
                                new Serializer(stream, new Revision(head)).struct(null,
                                    StaticMeshInfo.class);
                    }
                }
                ...
                if (this.method.equals(SerializationType.ENCRYPTED_BINARY))
                {
                    ...
                    if (size % 4 != 0)
                        padding = 4 - (size % 4);
                    stream =
                        new MemoryInputStream(Crypto.XXTEA(stream.bytes(size + padding),
                            true));
                    stream.seek(padding);
                }
    ...
    public static byte[] compress(SerializationData data, boolean preferCompressed)
    {
        ...
        if (head >= 0x109 || isStaticMesh)
        {
            ...
            if (head >= 0x189 && !isStaticMesh)
            {
                ...
                if (head >= 0x297 || (revision.has(Branch.LEERDAMMER,
                ...
                new Serializer(stream, revision).struct(data.getStaticMeshInfo(),
                    StaticMeshInfo.class);
        }
    ...
    public static byte[] changeRevision(byte[] data, Revision revision)
    {
        ...
        try
        {
            ...
            if (struct instanceof RPlan plan)
            {
                Thing[] things = null;
                                  ...
                                  ResourceDescriptor newDescriptor)
    {
        ...
        if (this.type != ResourceType.STATIC_MESH)
        {
            ...
            if (type.equals(ResourceType.MUSIC_SETTINGS) || type.equals(ResourceType.FILE_OF_BYTES) || type.equals(ResourceType.SAMPLE) || isFSB)
            {
                ...
                {
                    oldDescBuffer =
                        Bytes.getIntegerBuffer(oldDescriptor.getGUID().getValue(),
                            this.compressionFlags);
                    newDescBuffer =
                        Bytes.getIntegerBuffer(newDescriptor.getGUID().getValue(),
                            this.compressionFlags);
                }
                else return;
            }
    ...
    {
        return this.dependencies.toArray(ResourceDescriptor[]::new);
    }
