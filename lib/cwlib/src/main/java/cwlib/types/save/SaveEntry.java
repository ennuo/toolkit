...
public final class SaveEntry extends FileEntry
{
    ...
    public SaveEntry(BigSave database, SaveEntry adventure, SlotID slotID, String path, long size
        , SHA1 sha1)
    {
        super(database, path, sha1, size);
    ...
    public BigSave getSave()
    {
        return (BigSave) this.source;
    }

    public SaveEntry getAdventure()
    {
    ...
    public boolean isAdventureLevel()
    {
        return this.adventure != null && this.slotID != null;
    }
    ...
    public byte[] extract()
    {
        return this.getSave().extract(this.getSHA1());
    }
    ...
    public void setDetails(byte[] buffer)
    {
        ...
        if (this.isAdventureLevel())
        {
            SerializedResource resource =
                new SerializedResource(archive.extract(this.adventure.getSHA1()));
            RAdventureCreateProfile profile =
                resource.loadResource(RAdventureCreateProfile.class);
            Slot slot = profile.getAdventureSlots().get(this.slotID);
            slot.root = new ResourceDescriptor(sha1, ResourceType.LEVEL);
            byte[] build = SerializedResource.compress(profile.build(resource.getRevision(),
        ...
        if (this.isLevel())
        {
            if (this.slot.isAdventure())
                this.slot.adventure = new ResourceDescriptor(sha1,
        ...
        if (this.isItem())
        {
            ...
            try
            {
                RPlan plan = new SerializedResource(buffer).loadResource(RPlan.class);
