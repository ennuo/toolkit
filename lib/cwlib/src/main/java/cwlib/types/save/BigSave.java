...
public class BigSave extends FileData
{
     ...
     */
    private final SaveArchive archive;
    ...
    public BigSave(File file)
    {
        ...
        if (this.archive.getArchiveRevision() == 5)
            throw new IllegalArgumentException("This is a Vita profile! Use the correct " +
                                               "load " +
                                               "option!");
        this.process();
    }
    ...
    private void process()
    {
        ...
        if (key.getRootType() != ResourceType.BIG_PROFILE)
            throw new IllegalArgumentException("Save archive doesn't have an RBigProfile " +
                                               "root " +
                                               "resource!");
        ...
        {
            item.details.translatedLocation =
                this.profile.stringTable.get(item.details.locationIndex);
            item.details.translatedCategory =
                this.profile.stringTable.get(item.details.categoryIndex);
            ResourceDescriptor plan = item.plan;
            if (plan == null || plan.isGUID()) continue;
            byte[] data = archive.extract(plan.getSHA1());
            if (data == null) continue;
            this.entries.add(new SaveEntry(this, item, this.generatePath(item), data.length,
                plan.getSHA1()));
        ...
        for (Slot slot : this.profile.myMoonSlots.values())
        {
            ...
            if (slot.isLevel())
            {
                String path = "levels/" + ((slot.name.isEmpty()) ? "Unnamed Level" :
                                               slot.name);
                this.entries.add(new SaveEntry(this, slot, path, data.length,
                    root.getSHA1()));
            ...
            if (slot.isAdventure())
            {
                String path = "adventures/" + ((slot.name.isEmpty()) ? "Unnamed Adventure" :
                                                   slot.name);
                SaveEntry adventure = new SaveEntry(this, slot, path, data.length,
                    root.getSHA1());
                ...
                {
                    profile =
                        new SerializedResource(data).loadResource(RAdventureCreateProfile.class);
                }
                ...
                for (Slot local : profile.getAdventureSlots().values())
                {
                    ...
                    String localPath = path + "/levels/" + ((local.name.isEmpty()) ?
                                                                "Unnamed " +
                                                                "Level" : local.name);
                    byte[] localData = this.archive.extract(local.root.getSHA1());
                    if (localData == null) continue;
                    this.entries.add(new SaveEntry(this, adventure, local.id, localPath,
                        localData.length, local.root.getSHA1()));
        ...
        for (Fat fat : this.archive)
        {
            ...
            byte[] data = fat.extract();
            this.entries.add(new SaveEntry(this, this.generatePath(data, sha1), fat.getSize(),
                sha1));
        }

        // When loading a Vita profile, everything gets concatenated in memory, so we have to
        // grab it from the queue.
        for (SHA1 sha1 : this.archive.getQueueHashes())
        {
            if (locked.contains(sha1)) continue;
            byte[] data = this.archive.extract(sha1);
            this.entries.add(new SaveEntry(this, this.generatePath(data, sha1), data.length
                , sha1));
        }
    ...
    public String generatePath(byte[] data, SHA1 sha1)
    {
        ...
        if (type == ResourceType.INVALID)
        {
            ...
            {
                case 0xFFD8FFE0:
                    return "resources/textures/" + sha1.toString() + ".jpg";
                case 0x89504E47:
                    return "resources/textures/" + sha1.toString() + ".png";
                default:
                    return "resources/unknown/" + sha1.toString();
            }
        }
    ...
    @Override
    public SaveEntry get(SHA1 sha1)
    {
        return this.lookup.get(sha1);
    }
    ...
    public void add(byte[] data)
    {
        ...
        if (method == SerializationType.BINARY)
        {
            ...
            {
                int result = JOptionPane.showConfirmDialog(null, "This file appears to be built " +
                                                                 "for a later version of " +
                                                                 "LittleBigPlanet, do you want to" +
                                                                 " convert it?", "Confirm",
                ...
                if (result == JOptionPane.YES_OPTION)
                {
                    ...
                    {
                        JOptionPane.showMessageDialog(null, "An error occurred converting the " +
                                                            "resource!", "Error",
            ...
            switch (resourceType)
            {
                case LEVEL:
                {
                    ResourceDescriptor root = new ResourceDescriptor(sha1,
                        ResourceType.LEVEL);

                    SlotID id = this.profile.getNextSlotID();

                    String version =
                        this.archive.getGameRevision().getSubVersion() > 0x105 ?
                            "_LBP3" : "_LBP1";
                    Crater crater =
                        Crater.valueOf("SLOT_" + (id.slotNumber % 82) + version);
                    ...
                    entry = new SaveEntry(this, slot, "levels/Unnamed Level",
                        data.length, sha1);
                ...
                case PLAN:
                {
                    ...
                    entry = new SaveEntry(this, item, this.generatePath(item),
                        data.length, sha1);
    ...
    {
        String path =
            "items/" + InventoryObjectType.getPrimaryName(item.details.type).toLowerCase() +
            "/";
        ...
        if (item.details.type.contains(InventoryObjectType.USER_PLANET))
        {
            ...
            else if (item.details.subType == InventoryObjectSubType.ADVENTURE)
                path += "adventure_maps/";
            else if (item.details.subType == InventoryObjectSubType.EXTERNAL)
                path += "external/";
        }
        ...
        if (item.details.type.contains(InventoryObjectType.USER_COSTUME) || item.details.type.contains(InventoryObjectType.COSTUME))
        {
            ...
            {
                boolean isDwarf =
                    (item.details.subType & InventoryObjectSubType.CREATURE_MASK_DWARF) != 0;
                boolean isGiant =
                    (item.details.subType & InventoryObjectSubType.CREATURE_MASK_GIANT) != 0;
                if (isDwarf && isGiant)
                    path += "bird/";
    ...
    public RBigProfile getProfile()
    {
        return this.profile;
    }

    public SaveArchive getArchive()
    {
        return this.archive;
    }
    ...
    public boolean save(File file)
    {
        if (this.archive.getArchiveRevision() == 5)
        {
            throw new UnsupportedOperationException("Saving Vita profiles isn't implemented" +
                                                    " yet!");
        }

        byte compressionFlags = CompressionFlags.USE_NO_COMPRESSION;
        Revision revision = this.archive.getGameRevision();
        if ((revision.getVersion() >= 0x297) || revision.has(Branch.LEERDAMMER,
            Revisions.LD_RESOURCES))
            compressionFlags = CompressionFlags.USE_ALL_COMPRESSION;

        this.archive.getKey().setRootHash(this.archive.add(SerializedResource.compress(this.profile,
