...
public final class DDSReader
{
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = 0; j < w; j++)
            {
                                ...
                                4 * i + k < height; k++)
                {
                    ...
                    pixels[4 * width * i + 4 * j + width * k] = getDXTColor(c0, c1, 255
                        , t0,
                        order);
                    if (4 * j + 1 < width)
                    {
                        pixels[4 * width * i + 4 * j + width * k + 1] =
                            getDXTColor(c0, c1, 255,
                                t1, order);
                        if (4 * j + 2 < width)
                        {
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = 0; j < w; j++)
            {
                                ...
                                4 * i + m < height; m++)
                {
                    ...
                    int t2 = (buffer[index] & 0x30) >> 4;
                    int t3 = (buffer[index++] & 0xC0) >> 6;
                    pixels[4 * width * i + 4 * j + width * m] = getDXTColor(c0, c1,
                        alphaTable[4 * m], t0,
                        order);
                    if (4 * j + 1 < width)
                    {
                        pixels[4 * width * i + 4 * j + width * m + 1] =
                            getDXTColor(c0, c1,
                                alphaTable[4 * m + 1], t1,
                                order);
                        if (4 * j + 2 < width)
                        {
                            pixels[4 * width * i + 4 * j + width * m + 2] =
                                getDXTColor(c0, c1,
                                    alphaTable[4 * m + 2],
                                    t2, order);
                            if (4 * j + 3 < width)
                                pixels[4 * width * i + 4 * j + width * m + 3] =
                                    getDXTColor(c0, c1,
                                ...
                                4 * i + k < height; k++)
                {
                    ...
                    if (4 * j + 1 < width)
                    {
                        ...
                        if (4 * j + 2 < width)
                        {
                            pixels[4 * width * i + 4 * j + width * k + 2] =
                                getDXTColor(c0, c1,
                                    getDXT5Alpha(a0, a1, alphaTable[4 * k + 2]), t2
                                    , order);
                            if (4 * j + 3 < width)
                                pixels[4 * width * i + 4 * j + width * k + 3] =
                                    getDXTColor(c0, c1,
    ...
    private static int getDXTColor(int c0, int c1, int a, int t, Order order)
    {
        switch (t)
        {
            ...
            case 2:
                return (c0 > c1) ? getDXTColor2_1(c0, c1, a, order) : getDXTColor1_1(c0,
                    c1, a,
                    order);
            case 3:
