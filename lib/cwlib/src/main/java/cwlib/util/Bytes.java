...
public final class Bytes
{
    ...
    {
        if (bytes == null)
            throw new NullPointerException("Can't convert null byte array to hexadecimal " +
                                           "string!");
    ...
    {
        int totalLength = 0;
        for (byte[] bytes : arrays) totalLength += bytes.length;
    ...
    public static byte[] replace(byte[] source, byte[] original, byte[] replacement)
    {
        ...
        for (int i = 0; i < offsets.length; ++i)
        {
            ...
            System.arraycopy(source, sourceOffset, buffer, sourceOffset + (diff * i),
                offset - sourceOffset);
