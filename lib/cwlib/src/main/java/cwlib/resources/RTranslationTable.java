...
public class RTranslationTable
{
    ...
    public RTranslationTable(byte[] data)
    {
        ...
        if (Bytes.toShortBE(data) == ((short) 0xFEFF))
        {
            ...
            {
                ex.printStackTrace();
                throw new SerializationException("Failed to parse RTranslationTable due " +
                                                 "to " +
                                                 "IOException!");
            }
            return;
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            String text = new String(stream.bytes(stringEnd - stream.getOffset()),
                StandardCharsets.UTF_16);
    ...
    {
        return this.translate(key & 0xFFFFFFFFL);
    }
    ...
    {
        RTranslationTable table = GsonUtils.fromJSON(FileIO.readString(Path.of(path)),
            RTranslationTable.class);
        return table;
    }
